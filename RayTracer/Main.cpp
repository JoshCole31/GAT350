...
int main(int, char**)
{
	...
	while (!quit)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			framebuffer->DrawRect(rand() % renderer->width, rand() % renderer->height, 100, 100, { (uint8_t)((rand()) * 255), (uint8_t)((rand()) * 255), 255, (uint8_t)((rand()) * 255) });
		...
		//framebuffer->DrawSimpleCurve(200, 200, 300, 100, 35, { 255,255,255,255 });
		//framebuffer->DrawQuadraticCurve(200, 200, 300, 100,400,200 ,15, { 255,255,255,255 });
		//framebuffer->DrawCircle(120,140,50 ,{ 255,255,255,255 });
		//framebuffer->DrawTriangle(300, 200, 40, 10, 50, 120, { 255,255,255,255 });
		////for homework
		//for (int i = 0; i < 100; i++)
		//{
