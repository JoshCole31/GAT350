...
Framebuffer::Framebuffer(Renderer* renderer, int width, int height)
{
    ...
    texture = SDL_CreateTexture(renderer->renderer, SDL_PIXELFORMAT_ABGR8888,
...
void Framebuffer::DrawPoint(int x, int y, const color_t& color)
{
    ...
    uint8_t invAlpha = 255 - alpha; //1(255) - alph
...
void Framebuffer::DrawCubicCurve(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int steps, const color_t& color)
{
    ...
    for (int i = 0; i < steps; i++) 
    {
        ...
        float b1 = 3.0f* (float)pow((1.0f - t1), 2.0f);
        float c1 = 3.0f * (float)pow((1.0f - t1), 2.0f);
        float d1 = (float)pow((t1), 3.0f);

        float a2 = (float)pow((1.0f - t2), 3.0f);
        float b2 = 3.0f * (float)pow((1.0f - t2), 2.0f);
        float c2 = 3.0f * (float)pow((1.0f - t2), 2.0f);
